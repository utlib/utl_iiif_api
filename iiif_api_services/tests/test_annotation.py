...
@override_settings()
class Annotation_Test_POST_WITHOUT_QUEUE(APIMongoTestCase):
    def setUp(self):
    ...
    def test_an_annotation_with_no_id_given_can_be_successfully_created(self):
        ...
        response = self.client.post("/"+settings.TOP_LEVEL_COLLECTION_NAME+"/annotation", data)
        if settings.QUEUE_POST_ENABLED:
            while self.client.get(response.data["status"]).status_code!=status.HTTP_301_MOVED_PERMANENTLY: pass # Wait till background process finishes
            response = self.client.get(response.data["status"]) 
        self.assertEqual(response.data["responseCode"], status.HTTP_201_CREATED)
...
@override_settings()
class Annotation_Test_POST_WITH_THREAD_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_POST_WITH_PROCESS_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_POST_WITH_CELERY_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_DELETE_WITHOUT_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_DELETE_WITHOUT_THREAD_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_DELETE_WITHOUT_PROCESS_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_DELETE_WITHOUT_CELERY_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_PUT_Without_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
    ...
    def test_an_annotation_can_be_updated_sucessfully(self):
        data = {"annotation": json.loads(open(ANNOTATION_MEDIUM).read())}
        data["annotation"]["label"] = "new_annotation1"
        data["annotation"]["viewingHint"] = "non-paged"
        response = self.client.put(URL+"/p0001-image", data)
        if settings.QUEUE_PUT_ENABLED:
            while self.client.get(response.data["status"]).status_code!=status.HTTP_301_MOVED_PERMANENTLY: pass # Wait till background process finishes
            response = self.client.get(response.data["status"]) 
        self.assertEqual(response.data["responseCode"], status.HTTP_200_OK)
        self.assertEqual(Annotation.objects.get(name="p0001-image").label, 'new_annotation1')
        self.assertEqual(Annotation.objects.get(name="p0001-image").viewingHint, 'non-paged')
...
@override_settings()
class Annotation_Test_PUT_With_THREAD_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_PUT_With_PROCESS_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
...
@override_settings()
class Annotation_Test_PUT_With_CELERY_QUEUE(APIMongoTestCase):
    def setUp(self):
        self.user = User.create_user('testadmin', 'testemail@mail.com', 'testadminpass', True)
        jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
        jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
