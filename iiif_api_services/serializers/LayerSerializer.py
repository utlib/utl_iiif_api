...
class LayerSerializer(rest_mongo_serializers.DocumentSerializer):
    id = CustomHyperlinkedIdentityField(view_name='layer-detail')
    context = rest_serializers.URLField(initial='http://iiif.io/api/presentation/2/context.json')
    type = rest_serializers.CharField(initial='oa:Layer')
...
class EmbeddedLayerSerializer(rest_mongo_serializers.EmbeddedDocumentSerializer):
    id = CustomHyperlinkedIdentityField(view_name='layer-detail')
    type = rest_serializers.CharField(initial='oa:Layer')
