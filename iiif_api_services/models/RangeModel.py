...
class Range(Document):
    ...
    order = fields.IntField(
        default=0, 
        help_text=("The order in which this (sub)Range was given when created from a main Manifest.")
    )
    embeddedEntirely = fields.BooleanField(
        default=False, 
        help_text=("Whether this Range should embedded in its entirety in the response.")
    )
    ...
    startCanvas = fields.URLField(
        null=True, 
        help_text=("A link from a range or range to a canvas that is contained within the range.")
    )
