...
class Layer(Document):
    ...
    order = fields.IntField(
        default=0, 
        help_text=("The order in which this (sub)Layer was given when created from a main Canvas.")
    )
    embeddedEntirely = fields.BooleanField(
        default=False, 
        help_text=("Whether this Layer should embedded in its entirety in the response.")
    )
