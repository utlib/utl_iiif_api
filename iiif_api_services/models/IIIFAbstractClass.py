...
class IIIFModel(Document):
    ...
    order = fields.IntField(
        default=0,
        help_text=(
            "The order in which this (sub)object was given when created from a parent object.")
    )
    embeddedEntirely = fields.BooleanField(
        default=False,
        help_text=(
            "Whether this object should be embedded in its entirety in the response.")
    )
    ...
    hidden = fields.BooleanField(
        default=False,
        help_text=("Whether to show this resource in the response.")
    )
    ...
    ATid = fields.StringField(
        ...
        required=True,
        help_text=(
            "The URI that identifies the resource. It is recommended that an HTTP URI be used for all resources.")
    )
