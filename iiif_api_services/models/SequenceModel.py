...
class Sequence(Document):
    ...
    order = fields.IntField(
        default=0, 
        help_text=("The order in which this (sub)Sequence was given when created from a main Manifest.")
    )
    embeddedEntirely = fields.BooleanField(
        default=False, 
        help_text=("Whether this Sequence should embedded in its entirety in the response.")
    )
    ...
    startCanvas = fields.URLField(
        null=True, 
        help_text=("A link from a sequence or range to a canvas that is contained within the sequence.")
    )